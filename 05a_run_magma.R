...
g_marker <- read.csv('/sc/arion/projects/roussp01a/liting/Olf/data/cell_DEGmarkers.csv',header=T)
g_marker$Class <- gsub('__','_',gsub("\\.| |/|\\,|\\'" ,'_',g_marker$Class))
...
g_marker_list <- split(g_marker$Gene, g_marker$Class )
sub_tf_targets_rep_s <- unique(sub_tf_targets_rep_s[,c('Gene','OSN')])

g_tf_shared_list <- split(sub_tf_targets_rep_s$Gene, sub_tf_targets_rep_s$OSN )# shared or specific TF's targets
...
g_marker_ensg <- lapply(g_marker_list, function(x){unique(subset(ensemblGeneInfo,HGNC.symbol %in% x)[,"Ensembl.Gene.ID"])}) # not by trend, pnly oevrlap or nonoverlap
g_tf_shared_ensg <- lapply(g_tf_shared_list, function(x){unique(subset(ensemblGeneInfo,HGNC.symbol %in% x)[,"Ensembl.Gene.ID"])}) # not by trend, pnly oevrlap or nonoverlap
...
# myAnalysis <- calcMagmaGeneSetPvals(outDir="/sc/arion/projects/CommonMind/liting/ENT/magma_cell_marker",
#                                     geneMetaSets=g_marker_ensg,
#                                     pvalInfo=grabGenePvalInfo(selectedGeneTypeName="ensemblProtCodGenes35kb10kbAutosomesNoBmhc"),
#                                     checkIfOutDirExists=F)


myAnalysis <- calcMagmaGeneSetPvals(outDir="/sc/arion/projects/CommonMind/liting/ENT/magma_tf_shared_targets",
                                    geneMetaSets=g_tf_shared_ensg,
                                    pvalInfo=grabGenePvalInfo(selectedGeneTypeName="ensemblProtCodGenes35kb10kbAutosomesNoBmhc"),
                                    checkIfOutDirExists=F)
